...
export default function Post({ post, onPostUpdate, indexPost }) {
  ...
  const likeHandler = async () => {
    try {
      ...
      const updated = res.data.likes.length; // Get the updated like count from the response
      socket.on("newLike", updated);

      if (isLiked) {
        setLike(updated.likes.length);
        socket.off("newLike", updated);
      socket.disconnect();
      }
