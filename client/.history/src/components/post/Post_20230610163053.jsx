...
export default function Post({ isPost, onPostUpdate, indexPost }) {
  ...
  //++++++++++++++++++ fetch Data +++++++++++++++++++
  useEffect(() => {
    socket.on("likeUpdated", ({ postId, member_id, likesCount }) => {
      if (postId === post.id) {
        setLike((prevLike) => {
          // If the current user liked the post and another user unliked it
          if (prevLike > likesCount && member_id === currentUser.member_id) {
            return prevLike - 1;
          }
          // If the current user unliked the post and another user liked it
          if (prevLike < likesCount && userId === currentUser.member_id) {
            return prevLike + 1;
          }
          // Otherwise, update the like count based on the server value
          return likesCount;
        });

        setIsLiked((prevIsLiked) => {
          // Toggle the isLiked state only if the current user's like status changed
          if (userId === currentUser.member_id) {
            return !prevIsLiked;
          }
          return prevIsLiked;
        });
  ...
  const likeHandler = async () => {
    try {
      ...
      // Emit a socket event to update the like count on other clients
