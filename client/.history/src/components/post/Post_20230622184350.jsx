...
export default function Post({ isPost, onPostUpdate, indexPost }) {
      ...
      unsubscribeUser = userRef.onSnapshot((userSnapshot) => {
        if (!userSnapshot.empty) {
          ...
          if (post.likes.length !== updatedPost.likes.length) {
            const messageData = {
              name: `${updatedPost.firstName} ${updatedPost.lastName}`,
              title: "Like post",
              profilePicture: "updatedPost",
              body: "",
              timestamp: new Date(),
            };
            dispatch({
              type: "MESSAGE_UPDATE",
              payload: [...messageUser, messageData],
            });
  ...
  }, [post.id, post.likes, currentUser.member_id, firestore, dispatch, messageUser, setLike, setIsLiked, setPost]);
