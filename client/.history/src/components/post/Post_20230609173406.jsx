...
export default function Post({ post, onPostUpdate, indexPost }) {
  ...
  useEffect(() => {
    ...
    // Define the event handler for receiving new like updates
    const handleNewLike = (newLike) => {
      // Update the local state variables based on the new like
      setLike((prevLike) => prevLike + newLike.likeCount);
      setIsLiked(newLike.isLiked);
    };
  
    // Subscribe to the "newLike" event
