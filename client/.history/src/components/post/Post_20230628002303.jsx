...
export default function Post({ isPost, onPostUpdate, indexPost }) {
  ...
  const SortableItem = SortableElement(({ item, imageCount }) => {
    const isImage = item.type === "image";

    if (isImage) {
      const IMAGE = item.url.toString();
      const setImage = (image) => {
        return {
          title: item.type,
          img: image.toString(),
          imgThumbnail: image.url.toString(),
        };
      };

      const GROUP_NUM_IMAGE = [
        ...
        [
          [setImage(IMAGE), setImage(IMAGE), setImage(IMAGE)],
          [setImage(IMAGE), setImage(IMAGE), setImage(IMAGE)],
          [setImage(IMAGE), setImage(IMAGE), setImage(IMAGE)],
        ],
        // Add code for additional image counts if needed
        // ...
      ];

      const image_Count = Math.min(imageCount, GROUP_NUM_IMAGE.length);
      const images = GROUP_NUM_IMAGE[image_Count === 0 ? 0 : image_Count - 1];
  ...
  const SortableList = SortableContainer(({ items }) => {
    ...
    if (items.length >= 1 && items[0].url) {
      ONE.push([setImage(items[0].url.toString())]);
    }

    const TWO = [];
    if (items.length >= 2 && items[0].url && items[1].url) {
      TWO.push([
        setImage(items[0].url.toString()),
        setImage(items[1].url.toString()),
      ]);
    }

    const THREE = [];
    if (items.length >= 3 && items[0].url && items[1].url && items[2].url) {
      console.log(items[0].url)
      THREE.push([
        setImage(items[0].url.toString()),
        setImage(items[1].url.toString()),
        setImage(items[2].url.toString()),
      ]);
    }
    ...
    const FOUR = [
      ...THREE.map((group) => {
        ...
        if (items.length >= 5 && items[4].url) {
          updatedGroup.push(setImage(items[4].url.toString()));
        }
        return updatedGroup;
      }),
    ];
    ...
    const FIVE = [
      ...FOUR.map((group) => {
        const updatedGroup = [...group];
        if (items.length >= 6 && items[5].url) {
          updatedGroup.push(setImage(items[5].url.toString()));
        }
        return updatedGroup;
      }),
    ];
    ...
    const MORE = [
      ...THREE.map((group) => {
        const updatedGroup = [...group];
        if (items.length >= 7 && items[4].url && items[5].url && items[6].url) {
          updatedGroup.push(
            setImage(items[4].url.toString()),
            setImage(items[5].url.toString()),
            setImage(items[6].url.toString())
          );
        }
        return updatedGroup;
      }),
      ...
      <Box style={{ "& > *": { height: "100%", width: "100%" } }}>
        {items.map((item, index) => {
          ...
          if (isImage) {
            ...
            const image_Count = Math.min(items.length, GROUP_NUM_IMAGE.length);
            const images = GROUP_NUM_IMAGE[image_Count - 1];

            return (
              <div className="shareImgItem" key={index}>
                <MuiFbPhotoGrid
                  images={images}
                  reactModalStyle={{ overlay: { zIndex: 2000 } }}
                />
            ...
            return (
              <div className="shareVideoItem" key={index}>
                <ReactPlayer
                  url={item.url}
                  className="shareVideo"
                  width="640"
