...
export default function Post({ post, onPostUpdate, indexPost }) {
  ...
  useEffect(() => {
    ...
    socket.on("newLike", (newLikeData) => {
      // Update the isLiked state only if the event is related to the current post
      if (newLikeData.postId === post.id) {
        setIsLiked(newLikeData.isLiked);
        setLike(newLikeData.likeCount);
      }
    });
  ...
  const likeHandler = async () => {
    try {
      ...
      const updatedIsLiked = res.data.likes.includes(currentUser.member_id);
