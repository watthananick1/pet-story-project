...
export default function Post({ isPost, onPostUpdate, indexPost }) {
  ...
  useEffect(() => {
    ...
    // Listen for postUpdate event from the server
  ...
  }, [currentUser.member_id, post, like]);
  ...
  const likeHandler = async () => {
    try {
      ...
      const updatedLikesCount = res.data.likes.length;
      setIsLiked(po.likes.includes(currentUser.member_id));

      // Update the local state optimistically
      setLike(updatedLikesCount);
      // setIsLiked(!isLiked);

      // Emit a likePost event to the server
      socket.emit("likePost", { postId: post.id, likes: updatedLikesCount });
    } catch (err) {
      console.log(err);
    }
  };
