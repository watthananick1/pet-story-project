...
export default function Post({ isPost, onPostUpdate, indexPost }) {
  ...
  const SortableItem = SortableElement(({ item }) => {
    ...
    if (isImage) {
      const setImage = (
        image: typeof IMAGE_VERTICAL | typeof IMAGE_HORIZONTAL
      ): IMuiFbPhotoGridImage => {
        return {
          title: item.type,
          img: item.url.toString(),
          imgThumbnail: item.url.toString(),
        };
      };
      type IGroupNumImage = IMuiFbPhotoGridImage[][];
      const ONE: IGroupNumImage = [
        [setImage(IMAGE_VERTICAL)],
        [setImage(IMAGE_HORIZONTAL)],
      ];
      ...
      const FOUR: IGroupNumImage = [
        ...THREE.map((group) => [...group, setImage(IMAGE_HORIZONTAL)]),
      ];

      const FIVE: IGroupNumImage = [
        ...FOUR.map((group) => [...group, setImage(IMAGE_VERTICAL)]),
      ];

      const MORE: IGroupNumImage = [
        ...THREE.map((group) => [
          ...group,
          setImage(IMAGE_HORIZONTAL),
          setImage(IMAGE_VERTICAL),
      ...
      // IMAGES = [
      //   { title: item.type, img: item.url.toString(), imgThumbnail: item.url.toString() }
      // ];
      return (
