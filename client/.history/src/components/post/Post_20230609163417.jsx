...
export default function Post({ post, onPostUpdate, indexPost }) {
  ...
  useEffect(() => {
    ...
    // Define a handler function for the "likesUpdated" event
    const handleLikesUpdated = (updatedLikes) => {
      setIsLiked(updatedLikes.includes(currentUser.member_id));
    };

    // Subscribe to the "likesUpdated" event and call the handler function
    socket.on("likesUpdated", handleLikesUpdated);

    // Clean up by unsubscribing from the "likesUpdated" event when the component unmounts
    return () => {
      socket.off("likesUpdated", handleLikesUpdated);
    };
  }, [currentUser.member_id, post.likes]);

  const handleLike = () => {
    // Perform like operation here
    // ...

    // Update the likes array
    const updatedLikes = [...post.likes, currentUser.member_id];
    setIsLiked(true);
  ...
  const likeHandler = async () => {
    ...
    // Update the local state variables based on the current state
    if (isLiked) {
      setLike(like - 1); // Decrease the like count by 1
    } else {
      setLike(like + 1); // Increase the like count by 1
    }
