...
export default function Post({ isPost, onPostUpdate, indexPost }) {
      ...
      if (id === post.id) {
        setLike((prevLike) => {
          if (prevLike !== likesCount && member_id === currentUser.member_id) {
            // If the current user's like count is not the same as the server's value
            return likesCount;
          }
          return prevLike;
        });
  ...
  useEffect(() => {
    const unsubscribe = firestore
      .collection("Posts")
      .doc(post.id) // Replace 'postId' with the actual ID of the post
      .onSnapshot((snapshot) => {
        const updatedPost = snapshot.data();
        const user = firestore.collection("User").where("member_id", "==", updatedPost.member_id).get().data()
        console.log(user);
        setPost(updatedPost);
      });
