      ...
      <Box style={{ "& > *": { height: "100%", width: "100%" } }}>
        {items.map((item, index) => {
          ...
          if (isImage) {
            ...
            const GROUP_NUM_IMAGE = Array.from({ length: imageCount }, (_, i) => {
              return Array.from({ length: i + 1 }, () => setImage(IMAGE));
            });

            const image_Count = Math.min(items.length, GROUP_NUM_IMAGE.length);
            const images =
              GROUP_NUM_IMAGE[image_Count === 0 ? 0 : image_Count - 1];
      ...
      <Card className="postWrapper">
        ...
        <CardContent>
          ...
          {/* <Grid container spacing={3}>
            <Grid key={imageIndex} item xs={group.length}></Grid>
          </Grid> */}
          <SortableList
