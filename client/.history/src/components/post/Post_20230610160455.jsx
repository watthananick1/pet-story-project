...
export default function Post({ isPost, onPostUpdate, indexPost }) {
  ...
  //++++++++++++++++++ fetch Data +++++++++++++++++++
  useEffect(() => {
    socket.on('likeUpdated', ({ id, useEffect(() => {
      socket.on('likeUpdated', ({ postId, userId }) => {
        if (postId === post.id && userId === currentUser.member_id) {
    ...
    }, [currentUser.member_id, isLiked, post.id]);
     }) => {
      if (postId === post.id && userId === currentUser.member_id) {
        setLike((prevLike) => prevLike + (isLiked ? -1 : 1));
        setIsLiked((prevIsLiked) => !prevIsLiked);
  ...
  const likeHandler = async () => {
    try {
      ...
      socket.emit('likeUpdate', { id: post.id, member_id: currentUser.member_id });
    } catch (err) {
      console.log(err);
    }
