...
export default function Post({ post, onPostUpdate, indexPost }) {
  ...
  const likeHandler = async () => {
    try {
      ...
      const updatedLikeCount = res.data.likes.length; // Get the updated like count from the response
  
      if (isLiked) {
        setLike(updatedLikeCount - 1); // Decrease the like count by 1
      } else {
        setLike(updatedLikeCount); // Set the updated like count
      }
