...
export default function Topbar() {
  ...
  const handleClicsetMessageDataLength(updatedMessageUser.filter(event => event.read === false).length);kNoti = () => {
    setAnchorElNoti(true);
    setMessageDataLength(0);
  };
  ...
  useEffect(() => {
    const unsubscribeNotification = firestore
      ...
      .onSnapshot((querySnapshot) => {
        ...
        setMessageDataLength(updatedMessageUser.filter(event => event.read === false).length);
      });
