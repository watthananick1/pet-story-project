...
export default function FormDialogImage({ onOpen }) {
  ...
  const [crop, setCrop] = useState({ unit: "%", width: 30, aspect: 1 / 1, rotation: 0, x: 0, y: 0 });
  ...
  const handleAvatarUpload = async () => {
    if (crop.width && crop.height) {
      const croppedImageBlob = await getCroppedImageBlob(avatarFile, crop, "newFile.jpeg");
    ...
    return new Promise((resolve) => {
      ...
      reader.onloadend = () => {
        ...
        image.onload = () => {
          ...
          const rotatedWidth = crop.width * Math.abs(Math.cos((rotation * Math.PI) / 180)) +
            crop.height * Math.abs(Math.sin((rotation * Math.PI) / 180));
          const rotatedHeight = crop.width * Math.abs(Math.sin((rotation * Math.PI) / 180)) +
  ...
  return (
    <div>
      <Dialog open={ะพีำ} onClose={handleClose}>
