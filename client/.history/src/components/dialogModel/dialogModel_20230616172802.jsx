...
export default function FilePreviewer({ onClose }) {
  ...
  const cropperRef = useRef(null); // Use createRef for cropperRef
  ...
  ref={(input) => {
    if (input) {
      cropperRef.current = input; // Assign the input element to cropperRef
    }
  }}
/>
              <Button
                ...
                onClick={() => cropperRef.current.inputElement.click()} // Use cropperRef to trigger file input click
              >
                <span>Choose</span>
              </Button>
            </Stack>
            <Stack direction="row" spacing={2} alignItems="center" mt={2}>
              {imagePreview && (
                <Cropper
                  src={imagePreview}
                  ref={cropperRef} // Use cropperRef for the Cropper component
                  style={{
                    height: 400,
                    width: 400,
                    display: "flex",
