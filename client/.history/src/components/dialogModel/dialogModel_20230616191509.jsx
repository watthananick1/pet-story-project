...
export default function FilePreviewer({ onClose }) {
  ...
  const getCroppedImageBlob = async (sourceImageUrl, crop) => {
    const canvas = document.createElement('canvas');
        const scaleX = sourceImageUrl.naturalWidth / sourceImageUrl.width;
        const scaleY = sourceImageUrl.naturalHeight / sourceImageUrl.height;
        ...
        setCroppedImageBlob(base64Image);
  };
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      //   `${user.member_id}/profilePicture/${fileName}`
      // );
      const croppedImage = await getCroppedImageBlob(croppedImageBlob, crop);
      console.log("croppedImageBlob:", croppedImage);
