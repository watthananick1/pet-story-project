...
const NestedModal = ({ onContent, onContentID, onCommentsID, onClose, onTitle, userId, onfirstName, onPostUpdate }) => {
  ...
  const handleSaveChanges = async () => {
    if (onContent !== content) {
      if (onTitle === "Post") {
        ...
        try {
            setLoading(true);
            await axios.put(`/api/posts/${onContentID}/comments/${onCommentsID}`, { member_id: userId, content: content });
            onPostUpdate(); // Invoke the callback function to re-fetch post data
          } catch (err) {
            console.log(err);
          } finally {
