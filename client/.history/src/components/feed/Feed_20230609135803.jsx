  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        const sortedPosts = res.data.sort((a, b) => {
          ...
          const timeDiffA = Math.abs(currentTime - new Date(a.createdAt).getTime());
          const timeDiffB = Math.abs(currentTime - new Date(b.createdAt).getTime());
    ...
    const fetchUserPosts = async () => {
      try {
        ...
        const sortedPosts = resใกฟะฟ.sort((a, b) => {
          const timeDiffA = Math.abs(currentTime - new Date(a.createdAt.seconds * 1000).getTime());
          const timeDiffB = Math.abs(currentTime - new Date(b.createdAt.seconds * 1000).getTime());
          return timeDiffA - timeDiffB;
        });
        setPosts(sortedPosts);
      } catch (error) {
